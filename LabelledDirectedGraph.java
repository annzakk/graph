	...
	public class LabelledDirectedGraph {
		...
		public void addEdge(Node p, Edge e) {
			if(graph.containsKey(p)) {
				if(!graph.containsKey(e.getName())) {
					System.out.println("Node " + e.getName().getId() + " is not in graph");
					return;
				}
		...
		public void addEdgesList(Node p, ArrayList<Edge> eList) {
			if(graph.containsKey(p)) {
				...
				for(int i=0; i < eList.size(); i++) {
					if(!graph.containsKey(eList.get(i).getName())) {
						System.out.println("Node " + eList.get(i).getName().getId() + " is not in graph");
					}
					else {
		...
		public void deleteEdge(Node p, Edge e) {
			if(graph.containsKey(p)) {
				if(graph.get(p).contains(e)) {
					...
					System.out.println("Node " + p.getId() + " is not linked to node " + e.getName().getId());
				}
			}
			else {
				System.out.println("Node " + p.getId() + " is not in graph");
		...
		public void printNodes() {
			...
			while (iter.hasNext()) {
			    ...
			    for(int i=0; i<neigh_list.size(); i++) {
			    	System.out.print(neigh_list.get(i).getName().getId() + " ");
